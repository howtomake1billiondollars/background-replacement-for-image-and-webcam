...
#             # background_image_path = '.\\uploads'
#             background_image_path = f"{UPLOAD_FOLDER}/{uploaded_file.filename}"
#             # print(f".{UPLOAD_FOLDER}/{uploaded_file.filename}")
...
def index():
    if request.method == 'POST':
        ...
        if uploaded_file.filename != '':
            background_image_path = f"{UPLOAD_FOLDER}/{uploaded_file.filename}"
            uploaded_file.save(background_image_path)
            session['background_image_path'] = background_image_path
            return redirect(url_for('video_feed'))  # Redirect to the video feed page
    return render_template('index.html')
...
def video_feed():
    ...
    if background_image_path:
        return Response(gen_frames(background_image_path), mimetype='multipart/x-mixed-replace; boundary=frame')
    else:
        # Redirect back to the index page if no background image is selected
...
def gen_frames(background_image_path):
    while True:
        # Read a frame.
        ok, frame = camera.read()
